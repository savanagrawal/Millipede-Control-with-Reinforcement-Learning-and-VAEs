...
the implementation of a reinforcement learning algorithm for controlling an ant in the circular direction. It utlises the Gymnasium [1] Mujoco 
Ant-v4 environment [2] and Sample Factory library [3]. With the help of wrapper class, the script rewards the agent for being at same co-ordinates
and circulating in same position. 
...
class CustomAntEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        It tries to fetch the co-ordinates of the ant at each timestep and gives reward based on the co-ordinates. 
        ...
        x_pos = observation[0]  # 0 index is x-coordinate after setting exclude_current_positions_from_observation=False
        y_pos = observation[1]  # 1 index is y-coordinate after setting exclude_current_positions_from_observation=False
        ...
        # Here, smaller the distance, higher the reward. 
...
def main():
    ...
    argv=["--env=Ant-v4", "--device=cpu", "--experiment=ant_circular_task", "--train_dir=./src/rl-training/train_dir"]
