...
the implementation of a reinforcement learning algorithm for controlling an ant in the y direction. It utlises the Gymnasium [1] Mujoco 
...
the y direction. The step function is overridden to modify the rewards and penalties based on the agent's actions. Specifically, the 
agent is encouraged to move in the y direction. This is done by calculating the change in y position between the current and previous 
steps and subtracting it from the original reward i.e., change in x position. As a result, if the agent moves in the positive x 
direction, the reward decreases, effectively encouraging the agent to move in the y direction.
