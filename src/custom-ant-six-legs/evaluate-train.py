...
Ant-v4 environment [2], custom six legs ant [4] and Sample Factory library [3]. With the help of wrapper class, the experiments are 
trained and saved. The saved experiment can be passed in enjoy function provided by Sample Factory library which renders the 
...
def get_experiment():
    ...
    '''
    experiments = ['ant_sixlegs_x_direction_task', 'ant_sixlegs_x_opp_direction_task', 
                   'ant_sixlegs_y_direction_task', 'ant_sixlegs_y_opp_direction_task']
...
def main():  
    ...
    argv = ["--env=gym_mujoco_ant_six_legs/AntSixLegs-v0", f"--experiment={experiment}", "--train_dir=./src/custom-ant-six-legs/train_dir"]
    # argv=["--env=gym_mujoco_ant_six_legs/AntSixLegs-v0", "--device=cpu", "--experiment=ant_sixlegs_x_direction_task",
