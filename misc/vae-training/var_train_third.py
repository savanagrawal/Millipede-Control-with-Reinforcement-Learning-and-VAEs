...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        z = F.leaky_relu(self.linear1(z)) #use leaky relu or try other functions
        z = self.linear2(z) #use linear
...
class VariationalAutoencoderTraining:
    ...
    def load_data(self):
        ...
        labels = np.zeros(len(self.actions))
        ...
        dataset = LabeledDataset(self.actions, self.labels)
...
def main():
    ...
    num_features = vae_training.actions.shape[-1] * vae_training.actions.shape[-2]
    latent_dims = 2
    vae = VariationalAutoencoder(num_features, latent_dims)

    # # Train the model
    vae = vae_training.train(vae, epochs=20)

    # Save the model
    vae_training.save_model(vae, './src/vae-training/assets/vae-train-third.pth')

    # Load the model
    vae = vae_training.load_model(vae, './src/vae-training/assets/vae-train-third.pth')
